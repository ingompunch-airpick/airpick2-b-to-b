  ...
  return (
    <div className="rounded-2xl border border-neutral-800 bg-neutral-900/60 overflow-hidden">
      ...
      <div className="p-3.5 flex items-center justify-between gap-2">
        <div className="min-w-0">
          <div className="flex items-center gap-2">
            ...
            <span className="text-[10px] text-zinc-500">({res.carModel})</span>
          </div>
        ...
        <div className="shrink-0">
          {saved ? (
            <span className="text-[9px] font-bold px-2.5 py-1.5 rounded-lg border text-emerald-400 bg-emerald-500/5 border-emerald-500/20 flex items-center gap-1">
            ...
            <button
              ...
              className="px-3 py-2 bg-amber-500 hover:bg-amber-400 active:scale-95 text-neutral-950 rounded-xl text-[10px] font-black flex items-center gap-1.5 transition-all disabled:opacity-50"
            >
              {isSaving ? (
      ...
      <div className="px-3.5 pb-3.5 space-y-2.5">
          ...
          >
            {isSaving ? (
              <>
                ...
                <span className="text-[10px] text-amber-500 font-bold">Storage 업로드 중…</span>
              </>
            ) : (
              <>
                <Camera className="text-amber-500/80" size={22} />
                <span className="text-[11px] font-black text-zinc-200">사진 추가하기</span>
                <span className="text-[9px] text-zinc-500">여러 장 선택 가능</span>
              </>
            )}
          </button>
        )}
        ...
        {/* 에러 */}
        {error && (
          <p className="text-[10px] text-rose-400 font-bold bg-rose-500/10 rounded-lg px-3 py-2">
            ⚠ {error}
          </p>
        )}
          ...
          <div className="grid grid-cols-4 gap-2 max-h-[200px] overflow-y-auto p-1 bg-neutral-950 rounded-xl border border-neutral-850">
            {previews.map((url, idx) => (
              ...
              >
                ...
                />
                <span className="absolute bottom-0.5 left-0.5 bg-black/70 px-1 rounded text-[7px] font-mono text-zinc-400">
                  #{idx + 1}
                </span>
                {!isSaving && (
                  <button
    ...
    <div className="min-h-screen bg-neutral-950 text-white p-5 pb-24">
      <div className="flex items-center gap-3.5 mb-5">
        ...
        <div>
          ...
          <p className="text-[10px] text-zinc-500 font-bold">사진 미등록 입고차량 후속 업로드</p>
        </div>
      </div>
