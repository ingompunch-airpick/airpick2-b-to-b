        ...
        <div className="p-4 border-b border-neutral-850 flex items-center justify-between bg-[#1C1C1E]">
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <span className="text-[8.5px] text-zinc-500 font-mono font-bold uppercase">DATABASE OVERRIDE WORKSTATION</span>
            </div>
          </div>
          <button 
            onClick={onClose}
            className="p-1.5 bg-neutral-950 hover:bg-neutral-850 text-zinc-400 rounded-xl text-[10px] font-black cursor-pointer border border-neutral-850"
          >
            <X size={12} />
          </button>
        </div>
        ...
        <div className="overflow-y-auto p-5 flex-1 space-y-4 max-h-[70vh]">
          ...
          <div className="p-3.5 bg-neutral-950 border border-neutral-850 rounded-2xl flex items-center justify-between">
            <div className="space-y-0.5">
              <span className="text-[9px] text-zinc-500 font-semibold uppercase block">고유 수납 코드 (Receipt ID)</span>
            ...
            <span className="text-[10px] text-amber-500 bg-amber-500/10 px-3 py-1 rounded-xl border border-amber-500/20 font-black">
          ...
          <div className="grid grid-cols-3 gap-3 text-xs font-sans">
            ...
            <div>
              <label className="text-[9.5px] font-bold text-zinc-500 block mb-1">주차공간 배정구획</label>
              <input 
                type="text" 
          ...
          <div className="p-4 bg-neutral-955 border border-neutral-850 rounded-2xl space-y-3 font-sans text-xs">
            <span className="text-[10px] font-extrabold text-amber-500 uppercase tracking-wider block">💸 정형 정밀 금융 수납 수동 조율</span>
            
            <div className="grid grid-cols-2 gap-3.5">
              <div>
                <label className="text-[9.5px] font-bold text-zinc-500 block mb-1">기본보관료 (공급가액)</label>
                <input 
                  type="number" 
                  value={editBasePrice || ''}
              ...
              <div>
                <label className="text-[9.5px] font-bold text-zinc-500 block mb-1">종합 발렛비 (대행 가산금)</label>
                <input 
                  type="number" 
            ...
            <div className="grid grid-cols-2 gap-3.5 pt-1">
              <div>
                <label className="text-[9.5px] font-bold text-zinc-500 block mb-1">초과요금 (연장일차 추가분)</label>
                <input 
                  type="number" 
              ...
              <div>
                <label className="text-[9.5px] font-bold text-zinc-500 block mb-1">할인요금 (차감액)</label>
                <input 
                  type="number" 
            ...
            <div className="bg-[#1C1C1E] border border-neutral-800/40 p-3 rounded-xl space-y-1.5 mt-2.5">
              ...
              <div className="flex justify-between items-center text-[10px] text-zinc-400 border-b border-neutral-800 pb-1.5 mb-1">
                <span>부가가치세 (10%)</span>
                <span className="font-mono text-zinc-450">{Math.round(calculatedNetTotal * 0.1).toLocaleString()}원</span>
              </div>
              <div className="flex justify-between items-center text-xs font-black">
                <span className="text-amber-500 flex items-center gap-1">
                  최종 합계 금액 <span className="text-[9px] text-zinc-500 font-normal">(VAT 포함)</span>
                </span>
          ...
          <div className="bg-neutral-950 border border-neutral-850 p-4 rounded-xl space-y-1.5 font-sans text-xs">
            <label className="text-[9.5px] font-bold text-zinc-500 block mb-1 uppercase tracking-wider font-semibold">가외 주유/차량 보관 정보 관리자 메모창</label>
            <textarea 
              value={editAdminMemo}
              onChange={e => setEditAdminMemo(e.target.value)}
              placeholder="고객 요청사안 및 주차 위치 상세사항을 추가적으로 정밀 기재하십시오."
              className="w-full h-16 px-3 py-2 bg-neutral-900 border border-neutral-800 rounded-xl text-zinc-300 font-semibold placeholder-zinc-750 outline-none focus:border-amber-500 text-[10.5px] leading-relaxed resize-none"
            />
          </div>
          ...
          <div className="bg-neutral-955 border border-red-955/40 p-4 rounded-xl space-y-3 font-sans text-xs">
            ...
            <div className="pt-1.5">
              {targetReservationForEdit.status !== 'request_out' && targetReservationForEdit.status !== 'completed_out' && targetReservationForEdit.status !== 'cancelled' ? (
                ...
                <div className="text-center py-2.5 bg-neutral-900 border border-neutral-850 rounded-lg text-zinc-500 text-[10px] font-bold">
