...
import { adminStatusBadgeClass, statusToLabel } from '../utils/reservationStatus';
    ...
    <div className="min-h-screen bg-neutral-950 text-white p-4 pb-24 font-sans relative">
      ...
      <div className="flex items-center gap-3.5 mb-5 select-none px-1">
        <div>
          ...
          <p className="text-[12px] text-zinc-500 font-bold uppercase tracking-wider">Active Parking Registrations & CRM</p>
        </div>
      </div>
      ...
      <div className="bg-gradient-to-r from-neutral-900 via-[#1C1C1E] to-neutral-900 p-4.5 rounded-2xl border border-neutral-800/50 shadow-xl mb-4.5">
        <div className="flex justify-between items-center mb-3">
          <span className="text-[12px] uppercase font-black tracking-wider text-zinc-400 flex items-center gap-1">
            <CreditCard size={12} className="text-amber-500" />
            접수 요약 현황
          </span>
          <span className="text-[11px] font-mono text-zinc-500 font-bold bg-[#1C1C1E] px-2 py-0.5 rounded border border-neutral-800">
            KST Realtime
          </span>
        </div>
        <div className="grid grid-cols-2 gap-4">
          <div className="bg-neutral-950/60 p-3 rounded-xl border border-neutral-800/40">
            <span className="text-[11.5px] text-zinc-500 font-bold block mb-0.5">전체 접수 건수</span>
            <span className="text-base font-black text-amber-500 font-mono">
              {totalCount.toLocaleString()}건
            </span>
          </div>
          <div className="bg-neutral-950/60 p-3 rounded-xl border border-neutral-800/40">
            <span className="text-[11.5px] text-zinc-500 font-bold block mb-0.5">총 결제금액</span>
      ...
      <div className="grid grid-cols-3 gap-2 p-1 bg-neutral-900 rounded-xl border border-neutral-800/50 mb-5 select-none text-center">
        <button
          ...
          className={`py-2 rounded-lg text-[12px] sm:text-xs font-bold transition-all flex flex-col items-center justify-center gap-1 leading-tight ${
        ...
        >
          <span>당일 예약 온손님</span>
          <span className={`text-[10.5px] px-1.5 py-0.2 rounded font-mono font-extrabold ${
      ...
      <div className="space-y-3">
        ...
        {filteredReservations.length === 0 ? (
          <div className="text-center py-12 bg-[#1C1C1E] rounded-2xl border border-neutral-850 p-6">
            ...
            <p className="text-[12px] text-zinc-600 mt-1 font-medium">검색어 철자 또는 차량번호 띄어쓰기를 다시 확인해 주세요.</p>
          </div>
        ) : (
          <div className="space-y-3">
                  ...
                  <div className="flex justify-between items-center pb-2.5 border-b border-neutral-800/35">
                    <div className="space-y-0.5">
                      <span className="text-xs font-black text-white tracking-tight font-mono flex items-center gap-1.5">
                        {res.carNumber}
                        {isVIP && (
                          <span className="text-[11.5px] px-1.5 py-0.2 bg-amber-500/10 text-amber-500 border border-amber-500/20 rounded font-black tracking-normal">
                            VIP 단골
                          </span>
                        )}
                      </span>
                      <span className="text-[12px] text-zinc-500 font-bold block">
                        {res.carModel}
                      </span>
                    </div>
                    <div>
                  ...
                  <div className="grid grid-cols-1 gap-y-1.5 text-xs text-zinc-300 font-sans">
                    <div className="flex justify-between items-center text-[13px]">
                      ...
                      <div className="flex items-center gap-1.5 font-mono">
                        ...
                        {res.departureTerminal === 'T1' ? (
                          <span className="text-[11px] px-1.5 py-0.2 rounded bg-[#00D2FF]/10 text-[#00D2FF] border border-[#00D2FF]/20 font-black">
                            1터미널
                          </span>
                        ) : (
                          <span className="text-[11px] px-1.5 py-0.2 rounded bg-[#FFB800]/10 text-[#FFB800] border border-[#FFB800]/20 font-black">
                            2터미널
                          </span>
                        )}
                      </div>
                    ...
                    <div className="flex justify-between items-center text-[13px]">
                      ...
                      <div className="flex items-center gap-1.5 font-mono">
                        ...
                        {isIndoor ? (
                          <span className="text-[11px] px-1.5 py-0.2 rounded-[6px] bg-[#A855F7] text-white font-black shrink-0 tracking-tight">
                            실내
                          </span>
                        ) : (
                          <span className="text-[11px] px-1.5 py-0.2 rounded-[6px] bg-[#22C55E] text-white font-black shrink-0 tracking-tight">
                            야외
                          </span>
                        )}
                      </div>
                    </div>
                    <div className="flex justify-between text-[13px] items-center pt-0.5">
                    ...
                    <div className="flex justify-between text-[13px] pt-1 border-t border-dashed border-neutral-800/85">
                      <span className="text-zinc-550 font-semibold">고객 정보</span>
                      <span className="font-bold text-zinc-200">
                        {res.userName} <span className="text-amber-500 text-[12px] font-mono font-bold">[이용: {visitCount}회]</span>
                      </span>
                    </div>
                  </div>
                </div>
            ...
            <div className="w-full max-w-md bg-[#1C1C1E] border border-neutral-800 rounded-t-3xl sm:rounded-3xl overflow-hidden shadow-2xl flex flex-col max-h-[90vh] text-zinc-100 animate-slide-up">
              ...
              <div className="p-5 border-b border-neutral-800/70 flex items-center justify-between bg-neutral-900/60">
                <div className="flex items-center gap-2.5">
                  ...
                  <div>
                    ...
                    <p className="text-[11.5px] text-zinc-500 font-bold uppercase tracking-wider">
              ...
              <div className="p-5 space-y-5 overflow-y-auto max-h-[60vh]">
                ...
                <div className="bg-gradient-to-br from-amber-500/5 to-amber-500/0 border border-amber-500/10 p-4 rounded-2xl flex items-center justify-between">
                  <div>
                    <span className="text-[11px] font-mono font-bold uppercase text-amber-500 tracking-wider">Customer Premium Rating</span>
                    <h4 className="text-sm font-black text-white">{selectedRes.userName} 고객님</h4>
                    <p className="text-[12px] text-zinc-400 font-semibold mt-0.5">{companyName} 누적 예약 건수: <span className="text-amber-500 font-bold font-mono">{visitCount}회</span></p>
                  </div>
                ...
                <div className="space-y-3">
                  <h4 className="text-[12px] font-black text-zinc-500 uppercase tracking-wider">항공편 및 관영 스케줄 매핑</h4>
                  ...
                  <div className="bg-neutral-950/40 border border-neutral-800/40 p-4 rounded-2xl space-y-3">
                    ...
                    <div className="grid grid-cols-2 gap-3 text-xs pt-0.5">
                      <div>
                        <span className="text-[12px] text-zinc-500 font-bold block mb-0.5">출국 터미널</span>
                      ...
                      <div>
                        <span className="text-[12px] text-zinc-500 font-bold block mb-0.5">항공편명</span>
                        <span className="font-black text-amber-400 font-mono tracking-wider">{depFlightCode}</span>
                      </div>
                    </div>
                    <div className="pt-2 text-[13px] border-t border-dashed border-neutral-800">
                  ...
                  <div className="bg-neutral-950/40 border border-neutral-800/40 p-4 rounded-2xl space-y-3">
                    ...
                    <div className="grid grid-cols-2 gap-3 text-xs pt-0.5">
                      <div>
                        <span className="text-[12px] text-zinc-500 font-bold block mb-0.5">입국 터미널</span>
                      ...
                      <div>
                        <span className="text-[12px] text-zinc-500 font-bold block mb-0.5">편명 (귀국)</span>
                ...
                <div className="space-y-3">
                  <h4 className="text-[12px] font-black text-zinc-500 uppercase tracking-wider">주문 및 차량상세</h4>
                ...
                <div className="space-y-3">
                  <h4 className="text-[12px] font-black text-zinc-500 uppercase tracking-wider font-mono">Mobile Contact Action</h4>
                  <div className="bg-neutral-950/30 border border-zinc-800 p-4 rounded-2xl flex items-center justify-between">
                    <div>
                      <span className="text-[11.5px] text-zinc-500 block font-bold">고객 안전 연락처</span>
                    ...
                    >
                      <PhoneCall size={14} />
                      <span className="text-[13px] font-black">즉시 통화</span>
                    </a>
                  </div>
                </div>
                  ...
                  <div className="flex gap-2 w-full">
                    ...
                    {selectedRes.status === 'completed_in' && (
                      <button
                        ...
                        className="flex-1 py-3 bg-red-950/85 hover:bg-rose-900/40 text-rose-450 border border-rose-500/20 rounded-xl text-[12.5px] font-black transition-all flex items-center justify-center gap-1"
                      >
                        🚨 강제 출고요청
                      </button>
                    )}
                    ...
                    {(selectedRes.status === 'completed_in' || selectedRes.status === 'request_out') && (
                      <button
                        ...
                        className="flex-1 py-3 bg-emerald-950/85 hover:bg-emerald-900/40 text-emerald-400 border border-emerald-500/20 rounded-xl text-[12.5px] font-black transition-all flex items-center justify-center gap-1"
                      >
                        ⚡ 강제 반납완료
                      </button>
                    )}
