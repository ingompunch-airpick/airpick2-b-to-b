    ...
    <div className="min-h-screen bg-neutral-950 text-white p-5 pb-24">
      ...
      <div className="flex items-center gap-3.5 mb-6">
        ...
        <div>
          ...
          <p className="text-[10px] text-zinc-500 font-bold uppercase">Payment Mutation Sync</p>
        </div>
      </div>

      {/* Digits Display Panel (Reference Image 5 styling) */}
      <div className="mb-6 space-y-2.5">
        <label className="text-[11px] font-black tracking-wide text-zinc-400 block text-center uppercase">
            ...
            return (
              <button
                ...
                className={`h-12 rounded-xl flex items-center justify-center font-bold tracking-tight text-sm font-mono transition-all border outline-none ${
                  isSpecial
                    ? btn === '전체삭제'
                      ? 'bg-red-550/10 border-red-500/20 text-red-500 hover:bg-neutral-800 text-[11px]'
      ...
      <div className="space-y-3.5">
        <div className="flex justify-between items-center px-1">
          ...
          {typedDigits && (
            <button 
              ...
              className="text-[9px] font-black text-amber-505 bg-amber-500/10 hover:bg-amber-500/20 px-2 py-1 rounded text-amber-500"
            >
              초기화
            </button>
          )}
        ...
        {typedDigits === '' ? (
                ...
                >
                  <div>
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-[10px] bg-neutral-950 text-zinc-400 font-medium px-2.5 py-1 rounded-md border border-neutral-800/80">
                        {res.carModel}
                      </span>
                    </div>
                    <div className="mt-1.5 flex items-center gap-2">
                      <p className="text-xs font-medium text-amber-500/90 select-all tracking-wide">{res.carNumber}</p>
                      <p className="text-[10px] text-zinc-500">인천공항 {res.departureTerminal}</p>
                    </div>
                  </div>

                  <div className="flex items-center gap-2">
                    <span className={`text-[9.5px] font-medium px-3.5 py-1.5 rounded-lg border uppercase tracking-tight flex items-center gap-1 ${payment.color}`}>
                      <PaymentIcon size={10} />
                      {payment.text}
                    </span>
          ...
          <div className="p-8 text-center bg-neutral-900 border border-neutral-800 rounded-2xl">
            ...
            <p className="text-[10px] text-zinc-650 mt-1">입력하신 "{typedDigits}" 번호를 다시 확인바랍니다.</p>
          </div>
        )}
      </div>
      ...
      {selectedRes && (
        <div className="mt-6 border-t border-neutral-800/80 pt-5 space-y-4 animate-in fade-in slide-in-from-bottom-3 duration-250">
          <div className="bg-neutral-900/60 border border-neutral-800 p-4 rounded-2xl">
            <p className="text-[10px] text-zinc-450 uppercase font-black tracking-wider">변경 타겟 정보</p>
            ...
            <span className="text-[10px] text-zinc-500 font-bold inline-block mt-1">
              현재 수정전 수납 방식: <strong className="text-zinc-300 font-black">{getMethodDetails(selectedRes.paymentMethod).text}</strong>
            </span>
          </div>

          <p className="text-[10px] font-black uppercase text-zinc-500 tracking-wider">수정 정산 수단 선택</p>
