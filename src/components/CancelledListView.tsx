    ...
    >
      ...
      <div className="bg-neutral-900 border border-neutral-850 p-5 rounded-2xl">
        <div className="flex items-center justify-between mb-2">
          ...
          <button
            onClick={onBack}
            className="text-[10px] bg-neutral-950 hover:bg-neutral-800 text-zinc-300 font-bold px-3 py-1.5 rounded-xl border border-neutral-800 transition-colors cursor-pointer"
          >
            메인으로
          </button>
        </div>
        ...
        <div className="relative mt-4">
          ...
          {searchTerm && (
            <button
              ...
              className="absolute inset-y-0 right-0 flex items-center pr-3 text-zinc-500 hover:text-white text-[10px]"
            >
              지우기
            </button>
          )}
          ...
          {filtered.length > 0 ? (
            filtered.map((res, idx) => (
              ...
              >
                ...
                <div className="flex justify-between items-start">
                  <div className="space-y-1">
                    <div className="flex items-center gap-1.5 flex-wrap">
                      <span className="text-[10.5px] font-black text-rose-450 bg-rose-500/10 px-2 py-0.5 rounded-lg border border-rose-500/10 uppercase tracking-wider font-mono">
                        CANCELED
                      </span>
                      <span className="text-xs font-black text-zinc-100">{res.userName}</span>
                      <span className="text-[10px] text-zinc-500 font-mono font-semibold">({res.phone})</span>
                    </div>
                    <div className="flex items-center gap-2 pt-0.5">
                      <span className="text-[13px] font-extrabold text-amber-500 tracking-tight font-mono">
                        {res.carNumber}
                      </span>
                      <span className="text-[11px] font-bold text-zinc-400">{res.carModel || '일반 차종'}</span>
                    </div>
                  </div>
                  ...
                  <button
                    ...
                    className="flex items-center gap-1.5 px-3 py-1.5 bg-neutral-900 hover:bg-zinc-800 text-zinc-350 hover:text-amber-400 border border-neutral-800 rounded-xl text-[10.5px] font-black transition-all shadow-sm cursor-pointer"
                    title="접수 상태 복구"
                  >
                    <Undo2 size={11} />
                    <span>접수 복구</span>
                ...
                {/* Subdetails Panel */}
                <div className="grid grid-cols-2 gap-2 text-[10px] bg-neutral-950/40 p-3 rounded-xl border border-neutral-900">
                  <div className="space-y-1">
                    ...
                    <div className="text-zinc-300 font-medium leading-normal space-y-1">
                      <div className="flex items-center gap-1.5 flex-wrap">
                        ...
                        {res.departureTerminal === 'T1' ? (
                          <span className="text-[8.5px] px-1 rounded bg-[#00D2FF]/10 text-[#00D2FF] border border-[#00D2FF]/20 font-black">1터</span>
                        ) : (
                          <span className="text-[8.5px] px-1 rounded bg-[#FFB800]/10 text-[#FFB800] border border-[#FFB800]/20 font-black">2터</span>
                        )}
                      </div>
                      <div className="flex items-center gap-1.5 flex-wrap">
                ...
                <div className="bg-rose-500/5 border border-rose-500/10 p-3 rounded-xl text-[10px] space-y-1">
                  <div className="flex items-center gap-1 text-rose-400/90 font-bold">
                    <ShieldAlert size={11} className="shrink-0" />
                    <span>취소 사유 및 일자</span>
                    {res.cancelledAt && (
                      <span className="text-[9px] font-mono font-medium text-rose-450/70 ml-auto">
                        ({res.cancelledAt})
                      </span>
                    )}
                  </div>
                ...
                <div className="flex items-center justify-between text-[9.5px] text-zinc-500 font-mono border-t border-neutral-900/60 pt-2.5">
            ...
            >
              ...
              <p className="text-[11px] font-bold">취소된 예약 내역이 존재하지 않습니다.</p>
              <p className="text-[9px] text-zinc-600">검색어를 지우거나 active 상태를 확인하십시오.</p>
            </motion.div>
          )}
        </AnimatePresence>
      </div>
